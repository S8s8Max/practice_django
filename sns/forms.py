...
# Friend form
class FriendForm(models.ModelForm):
    class Meta:
        model = Friend
        fields = ["owner", "user", "group"]

# Good form
class GoodForm(models.ModelForm):
    class Meta:
        model = Good
        fields = ["owner", "message"]

# Group check box form
class GroupCheckForm(models.Form):
    def __init__(self, user, *args, **kwargs):
        super(GroupCheckForm, self).__init__(*args, **kwargs)
        public = User.objects.filter(username="public").first()
        self.fields["groups"] = forms.MultipleChoiceField(
            choices=[(item.title, item.title) for item in \
                Group.objecets.filter(owner__in=[user,public])],
            widget=forms.CheckboxSelectMultiple(),
        )

# Group select form
class GroupSelectForm(models.Form):
    def __init__(self, user, *args, **kwargs):
        super(GroupSelectForm, self).__init__(*args, **kwargs)
        self.fields["groups"] = forms.ChoiceField(
            choices=[("-","-")] + [(item.title, item.title) \
                for item in Group.objects.filter(owner=user)],
                wiget=forms.Select(attrs={"class":"form-control"}),
        )
